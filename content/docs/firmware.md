...
Wi-Fi нужно настроить на начальном этапе, даже если функцию наблюдателя использовать не планируется: при подключении к Wi-Fi бридж фиксирует канал, на котором будет работать. Поэтому Wi-Fi обязателен для бриджа.

Настройки для репитера и бриджа — в разделах [Настройки для репитеров](/docs/settings/radio#настройки-для-репитеров) и [Настройки для бриджей](/docs/settings/radio#настройки-для-бриджей).
